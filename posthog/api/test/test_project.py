...
class TestProjectAPI(team_api_test_factory()):  # type: ignore
    ...
    @parameterized.expand(
        [
            ("missing", None),
            ("empty", []),
            ("limited", [{"key": "organizations_projects", "name": "Projects", "limit": 1}]),
        ]
    )
    def test_self_hosted_project_creation_without_billing(self, _name: str, features: list[dict] | None) -> None:
        ...
        with self.is_cloud(False):
            organization_response = self.client.get(f"/api/organizations/{self.organization.id}/")
            self.assertEqual(organization_response.status_code, status.HTTP_200_OK)
            projects_features = [
                feature
                for feature in organization_response.json()["available_product_features"]
                if feature["key"] == AvailableFeature.ORGANIZATIONS_PROJECTS
            ]
            self.assertEqual(len(projects_features), 1)
            self.assertIsNone(projects_features[0]["limit"])
            for name in ["Second project", "Third project"]:
                response = self.client.post("/api/projects/", {"name": name})
                self.assertEqual(response.status_code, status.HTTP_201_CREATED)
