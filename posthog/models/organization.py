...
class Organization(ModelActivityMixin, UUIDTModel):
    ...
    def get_effective_product_features(self) -> list[ProductFeature]:
        features = self.available_product_features or []
        if is_cloud():
            return features
        return [
            feature for feature in features if feature and feature.get("key") != AvailableFeature.ORGANIZATIONS_PROJECTS
        ] + [
            {
                "key": AvailableFeature.ORGANIZATIONS_PROJECTS,
                "name": "Projects",
                "description": "Create multiple projects in your organization.",
                "unit": None,
                "limit": None,
                "note": None,
    ...
    def get_available_feature(self, feature: Union[AvailableFeature, str]) -> ProductFeature | None:
        return next(
            filter(lambda f: f and f.get("key") == feature, self.get_effective_product_features()),
            None,
        )
